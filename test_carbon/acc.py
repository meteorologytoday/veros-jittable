...
class ACCSetup(VerosSetup):
    ...
    def set_parameter(self, state):
        ...
        register_bgc_tracer(state, "tracer", units="mol/m^3", long_name="Custom passive tracer")
        ...
        var_meta.update(
            ...
            tracer_star=Variable("tracer_star", ("yt",), "mol/m^3", "Reference surface tracer concentration"),
            tracer_rest=Variable("tracer_rest", ("xt", "yt"), "1/s", "Surface tracer restoring time scale"),
        )
    ...
    def set_initial_conditions(self, state):
        ...
        vs.tracer_star = npx.where((vs.yt > 10) & (vs.yt < 15), 1.0, 0.0)
        vs.tracer_rest = vs.dzt[npx.newaxis, -1] / (30.0 * 86400.0) * vs.maskT[:, :, -1]
        ...
        # tracer: initialize to zero everywhere
        vs.tracer = update(vs.tracer, at[...], 0.0)

    @veros_routine
    def set_forcing(self, state):
        vs = state.variables
        vs.forc_temp_surface = vs.t_rest * (vs.t_star - vs.temp[:, :, -1, vs.tau])
        vs.forc_tracer_surface = vs.tracer_rest * (vs.tracer_star - vs.tracer[:, :, -1, vs.tau])
